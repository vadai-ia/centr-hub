...
- **Workaround / fix:** `REVOKE EXECUTE ON FUNCTION public.<fn> FROM PUBLIC; GRANT EXECUTE ON FUNCTION public.<fn> TO <roles necesarios>;`. Para `bootstrap_organization`: solo `service_role` (migración 0010 lo aplicó; 0011 lo re-aplica para idempotencia). Para `is_member_of`: `anon, authenticated` — ambos roles necesitan EXECUTE para que las RLS policies que llaman a esta función no fallen con "permission denied for function". Aplicado en migración 0011.
- **Lección:** Al crear cualquier función SECURITY DEFINER, el cierre siempre incluye: (1) `REVOKE EXECUTE ... FROM PUBLIC`, (2) `GRANT EXECUTE ... TO <solo-los-roles-necesarios>`. Para funciones de admin/bootstrap: solo `service_role`. Para funciones usadas en RLS policies: también `authenticated` (y `anon` si alguna tabla pública usa la función en sus policies). El patrón de referencia está al final de `0010_seeds_bootstrap.sql`. Respecto a `rls_auto_enable`: si Security Advisor reporta una función no encontrada en migraciones propias, es probablemente una función Supabase-interna; no crear una custom — solo documentar y omitir.
