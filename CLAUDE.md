...
**% de cierre de cotizaciones — AUTOMÁTICO, sin objetivo.** Nació en 0053 como métrica de meta capturada (`close_rate`), pero la dirección pidió no poner el número: ahora **se calcula solo** para el equipo y cada asesor —cotizaciones del mes, cuántas ya se ganaron/pagaron y el % (100 cotizaciones y 30 pagadas = 30%)— y la meta de **cotizaciones enviadas** también se retiró por lo mismo. Solo `won` y `amount` llevan objetivo (`EDITABLE_GOAL_METRICS` en `lib/metas/schema.ts`); el schema rechaza capturar las otras dos, y el avance en vivo y el snapshot mensual ignoran las filas viejas de `quotes`/`close_rate` que sigan en `goals` (no hace falta borrarlas). Se ve en la tarjeta "Metas del mes" (columna "% cierre"), en Mi Día y en Admin → Metas → "Avance por mes", que ofrece los últimos 6 meses aunque no tengan metas: el % de un mes cerrado **no se congela**, se recalcula de los datos (`loadCloseRatesForMonth`), porque una cotización de julio pagada en septiembre sí mejora el cierre de julio. Su barra (`CloseRateBar`) **no usa el semáforo**: rojo/amarillo/verde miden avance contra un objetivo y aquí no lo hay. Es **por cohorte**: de las cotizaciones CREADAS en el mes, cuántas ya se ganaron (`opportunities.won_at` sobre la misma opp de la cotización). NO es "ganadas del mes ÷ cotizaciones del mes", que mezcla cierres de cotizaciones de meses anteriores y puede pasar de 100%. **Consecuencia a comunicar:** en el mes en curso arranca bajo y sube conforme cierran (medido en Centr: jul 51%, ago 53%, sep 38% a mitad de mes). La venta orgánica no lo tiene (no cotiza). La definición única vive en `lib/metas/achievement.ts` (`closeRateOf`), módulo puro que sale del mismo tally por cohorte que el desglose del Dashboard.
...
6. Cualquier error nuevo que descubras o workaround que aplique, agrégalo a `ERRORES.md` antes de cerrar el milestone. **Formato obligatorio (condensado):** cada entrada es `### Título` + bullets `Causa raíz:` y `Regla:` — **solo la lección accionable** (causa raíz + cómo evitarla). NO incluir narrativa de diagnóstico, fechas, hashes de commit, UUIDs/IDs de un solo uso, nombres de ejemplo, ni redundancia con entradas existentes (si la lección ya está, amplía esa entrada en vez de duplicar). SÍ preservar el invariante que protege el sistema (migración, tabla/columna/función, regla R#/O#) + referencia mínima al código. El estándar completo está en la cabecera de `ERRORES.md` ("Formato obligatorio de cada entrada").
